...
-- Dumping structure for table sms_db.auth_assignment
...
-- Dumping data for table sms_db.auth_assignment: ~0 rows (approximately)
INSERT INTO `auth_assignment` (`item_name`, `user_id`, `created_at`) VALUES
	('Administrator', 1, 1740245185);
...
INSERT INTO `concern_type` (`id`, `description`) VALUES
	(1, 'General');
...
CREATE TABLE IF NOT EXISTS `module_grant` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `module_list_id` int(11) NOT NULL DEFAULT '0',
  ...
  `student_id` int(11) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_module_grant_module_list` (`module_list_id`),
  CONSTRAINT `FK_module_grant_module_list` FOREIGN KEY (`module_list_id`) REFERENCES `module_list` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- Dumping data for table sms_db.module_grant: ~0 rows (approximately)

-- Dumping structure for table sms_db.module_list
CREATE TABLE IF NOT EXISTS `module_list` (
  ...
  `date_close` date DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_module_list_subject` (`subject_id`),
  CONSTRAINT `FK_module_list_subject` FOREIGN KEY (`subject_id`) REFERENCES `subject` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table sms_db.module_list: ~0 rows (approximately)
...
-- Dumping data for table sms_db.profile: ~1 rows (approximately)
...
-- Dumping data for table sms_db.token: ~3 rows (approximately)
...
-- Dumping data for table sms_db.user: ~3 rows (approximately)
